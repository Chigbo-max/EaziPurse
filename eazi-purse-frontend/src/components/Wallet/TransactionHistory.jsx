...
const TransactionHistory = () => {
  ...
  const getTransactionType = (transaction) => {
    if (transaction.transaction_type === 'D') {
      return transaction.sender ? 'Transfer Sent' : 'Deposit';
  ...
  const getTransactionIcon = (transaction) => {
    if (transaction.transaction_type === 'D' && !transaction.sender) {
      return <ArrowTrendingUpIcon className="w-5 h-5 text-success-400" />;
    } else if (transaction.transaction_type === 'T' || (transaction.transaction_type === 'D' && transaction.sender)) {
  ...
  const filteredTransactions = transactions?.filter(transaction => {
    ...
    if (filter === 'received') return transaction.receiver;
    if (filter === 'deposits') return transaction.transaction_type === 'D' && !transaction.sender;
    return true;
  }) || [];
      ...
      >
        ...
        <div>
          ...
          <p className="text-white/60 mt-1">
            View all your transfers and received funds
          </p>
        </div>
      </motion.div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-sm">
                      {transaction.sender 
                        ? `To: ${transaction.receiver?.first_name} ${transaction.receiver?.last_name}`
